...
/**
 * This application can be used as template for LbrMed Position and GMS Referencing.
...
public class PositionAndGMSReferencing extends RoboticsAPIApplication {
    ...
    {
        medController = (MedController) lbrMed.getController();
    }

    public void run()
    {
        PositionMastering mastering = new PositionMastering(lbrMed);
        ...
        if (OperationMode.T1 == lbrMed.getOperationMode())
        {
            joggingVelocity = 0.4;
        }
        ...
        if (allAxesMastered)
        {
            ...
            lbrMed.move(ptpHome().setJointVelocityRel(joggingVelocity));
    ...
    private void performMotion(JointPosition position)
    {
        ...
        lbrMed.move(mainMotion);

        getLogger().info("Moving to current position from negative direction");
        JointPosition position1 = new JointPosition(lbrMed.getJointCount());
        for (int i = 0; i < lbrMed.getJointCount(); ++i)
        {
        ...
        JointPosition position2 = new JointPosition(lbrMed.getJointCount());
        for (int i = 0; i < lbrMed.getJointCount(); ++i)
        {
    ...
    {
        ISunriseRequestService requestService = (ISunriseRequestService) (medController.getRequestService());
